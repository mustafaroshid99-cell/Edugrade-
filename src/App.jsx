...
const downloadMarksheet = async (student, project, showToast) => {
  ...
  headerY += 7;
  doc.text(etype, headerCenterX, headerY, { align: "center" });
  doc.setFontSize(15);
  headerY += 7;
  ...
  doc.autoTable({
    ...
    columnStyles: {
      ...
      6: { halign: "center", cellWidth: 13, textColor: TEAL, fontStyle: "bold" },
      7: { halign: "center", cellWidth: 28, fontStyle: "bold" },
      8: { halign: "center", cellWidth: 12, fontStyle: "bold" },
    },
    didParseCell: (data) => {
      if (data.section === "body") {
        if (data.column.index === 7) { // Obtained A+B — color by pass/fail threshold
  ...
  const row1 = [
    ...
    { label: "Total GP",               value: String(totalGP.toFixed(1)),   fillColor: TEAL_LT,   borderColor: [153,234,214], valueColor: TEAL },
  ];
  const row1H = 18;
  row1.forEach((box, bi) => {
  ...
  const row2 = [
    { label: "GPA",           value: String(student.gpa), fillColor: SKY_LT,    borderColor: SKY_MID, valueColor: SKY },
  ...
  doc.text(`${school}  —  ${etype} Progress Report`, 10, H-7.5);
...
const getFirebaseInstance = () => {
  ...
  try {
    const app = firebaseCompat.apps.length
      ? firebaseCompat.apps[0]
      : firebaseCompat.initializeApp(FIREBASE_CONFIG);
    const auth = firebaseCompat.auth(app);
    const db   = firebaseCompat.firestore(app);
