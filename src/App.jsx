...
const LogoSplash = ({ onDone }) => {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => onDoneRef.current(), 2350);
    return () => { clearTimeout(t1); clearTimeout(t2); };
  }, []); // empty deps: timer fires once on mount, never resets on re-renders
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "login") {
        ...
        const snap = await getDoc(doc(firebase.db, "users", cred.user.uid));
        ...
        await addDoc(collection(firebase.db, "adminNotifications"), {
...
const AdminPanel = ({ firebase, showToast, onClose }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const notifsQ = query(collection(firebase.db, "adminNotifications"), orderBy("createdAt", "desc"), fbLimit(50));
  ...
  const toggleBlock = async (user) => {
    ...
    await updateDoc(doc(firebase.db, "users", user.id), { blocked: newBlocked });
  ...
  const markAllRead = async () => {
    await Promise.all(
      notifications.filter(n => !n.read).map(n =>
        updateDoc(doc(firebase.db, "adminNotifications", n.id), { read: true })
      )
    );
  ...
  const loadUserFullData = async (user) => {
    try {
      const projQ = query(collection(firebase.db, "users", user.id, "projects"), orderBy("createdAt", "asc"));
...
// ─── PDF: Marksheet — Image-Accurate Format with Merit-List Colors ───────────
const downloadMarksheet = async (student, project, showToast) => {
  ...
  // ── Color Palette (matches Merit List) ────────────────────────────────────
  ...
  // ── Top gradient bar ──────────────────────────────────────────────────────
  doc.setFillColor(...SKY_DK);  doc.rect(5, 5, (W-10)/3, 6, "F");
  doc.setFillColor(...SKY);     doc.rect(5+(W-10)/3, 5, (W-10)/3, 6, "F");
  doc.setFillColor(...SKY_MID); doc.rect(5+2*(W-10)/3, 5, (W-10)/3, 6, "F");
  ...
  const headerCenterX = W / 2 - 14; // shifted left to leave room for grade table

  doc.setFont("helvetica", "bold");
  doc.setFontSize(22);
  doc.setTextColor(...SKY);
  doc.text(school, headerCenterX, 22, { align: "center" });
  ...
  if (loc) {
    ...
    headerY += 6;
    doc.text(loc, headerCenterX, headerY, { align: "center" });
  }
  ...
  headerY += 7;
  doc.text(project.examName || "", headerCenterX, headerY, { align: "center" });
  doc.setFontSize(15);
  headerY += 7;
  doc.text("PROGRESS REPORT", headerCenterX, headerY, { align: "center" });

  // ── Grade Interval Table (top right, matching image layout) ───────────────
  ...
  doc.rect(giX, giY, giW, 7, "F");
  doc.setFont("helvetica","bold"); doc.setFontSize(7); doc.setTextColor(...WHITE);
  doc.text("Grace Interval", giX + giW/2, giY + 4.8, { align: "center" });

  // Column header row
  const giColW = [24, 14, 14];
  const giColX = [giX, giX + 24, giX + 38];
  doc.setFillColor(...LGREY);
  doc.rect(giX, giY + 7, giW, 6, "F");
  doc.setFont("helvetica","bold"); doc.setFontSize(6); doc.setTextColor(...DARK);
  ["Grade Interval", "GPA", "LG"].forEach((h, ci) =>
    doc.text(h, giColX[ci] + giColW[ci]/2, giY + 11.5, { align: "center" })
  );

  // Divider lines between columns
  doc.setDrawColor(...SKY_MID); doc.setLineWidth(0.25);
  doc.line(giColX[1], giY, giColX[1], giY + 7 + 6 + sortedScale.length * giRH);
  doc.line(giColX[2], giY, giColX[2], giY + 7 + 6 + sortedScale.length * giRH);

  // Data rows — correct range calculation
  sortedScale.forEach((row, ri) => {
    const ry = giY + 13 + ri * giRH;
    ...
    // Range: lower = row.min, upper = (ri===0 ? 100 : sortedScale[ri-1].min - 1)
    const upper = ri === 0 ? 100 : sortedScale[ri-1].min - 1;
    const lower = ri === sortedScale.length-1 ? 0 : row.min;
    const rangeStr = `${String(lower).padStart(2,"0")} \u2013 ${upper}`;
    doc.text(rangeStr, giColX[0] + giColW[0]/2, ry + 4, { align: "center" });
    doc.text(String(row.gp), giColX[1] + giColW[1]/2, ry + 4, { align: "center" });
    doc.setFont("helvetica","bold");
    doc.setTextColor(...(["A+","A","A-"].includes(row.grade) ? GREEN : row.grade==="F" ? RED : DARK));
    doc.text(row.grade, giColX[2] + giColW[2]/2, ry + 4, { align: "center" });
  });

  // Outer border for grade table
  doc.setDrawColor(...SKY_MID); doc.setLineWidth(0.4);
  doc.rect(giX, giY, giW, 13 + sortedScale.length * giRH, "S");

  // ── Divider under header ──────────────────────────────────────────────────
  const divY = Math.max(headerY + 5, giY + 13 + sortedScale.length * giRH + 3);
  doc.setDrawColor(...SKY_MID); doc.setLineWidth(0.7);
  doc.line(8, divY, W-8, divY);

  // ── Student Information ───────────────────────────────────────────────────
  // Layout (matches image):
  //   Row 1: Student Name  :  ___________  (full left side)
  //   Row 2: Class         :  _______      |  Total Subjects :  ____
  //   Row 3: Roll No       :  _______      |  Total Marks    :  ____
  ...
  // totalObtained computed after tableBody (see below)
  ...
  const drawInfoRow = (label, value, x, y, lineEndX) => {
    ...
    doc.line(valX - 1, y + 1.5, lineEndX, y + 1.5);
  };

  // Row 1: Name (left, full width up to grade table)
  drawInfoRow("Student Name", student.name, lcol, siY, midCol + 38);

  // Row 2: Class (left) | Total Subjects (right)
  drawInfoRow("Class",          cls,                           lcol,   siY + rowGap,     midCol - 6);
  drawInfoRow("Total Subjects", String(project.subjectCount),  midCol, siY + rowGap,     W - 10);

  // Row 3: Roll No (left) | Total Marks (right)
  drawInfoRow("Roll No",    String(student.roll), lcol,   siY + rowGap*2, midCol - 6);
  drawInfoRow("Total Marks", String(totalFull),   midCol, siY + rowGap*2, W - 10);

  // ── Subject Marks Table ───────────────────────────────────────────────────
  // Calculation:
  //   Total (exam) = MCQ/Oral + Creative/Written
  //   B            = round(Total × examWeight%)
  //   Obtained (A+B) = CA + B
  //   GP based on (Obtained / FullMarks) × 100
  ...
  const tableBody = subjectNames.map((subName, i) => {
    ...
    const examTot = mcq + cre;                          // MCQ + Creative = exam total
    const B       = Math.round(examTot * exw / 100);   // B = Y% of exam total
    const obtained= Number((student.obtainedMarks||[])[i]) || (ca + B); // A + B
    const pct     = full > 0 ? (obtained / full) * 100 : 0;
    const gp      = gpFn(pct);
    return [subName || `Subject ${i+1}`, full, ca, mcq, cre, examTot, obtained, gp];
  });

  const totalObtained = tableBody.reduce((a, r) => a + Number(r[7]), 0);
  const tableStartY = siY + rowGap * 2 + 7;
  ...
  doc.autoTable({
    ...
    styles: {
      ...
      lineColor: SKY_MID, lineWidth: 0.3,
      valign: "middle",
    },
    headStyles: {
      fillColor: SKY, textColor: WHITE, fontStyle: "bold",
      fontSize: 7.5, halign: "center", valign: "middle", minCellHeight: 14,
    },
    ...
    didParseCell: (data) => {
      if (data.section === "body") {
        if (data.column.index === 6) { // Obtained A+B
          const v    = Number(data.cell.raw);
          const full = Number(tableBody[data.row.index]?.[1]) || 100;
          const pct2 = (v / full) * 100;
          data.cell.styles.textColor =
            pct2 >= 80 ? GREEN : pct2 < (project.passThreshold ?? 33) ? RED : DARK;
        }
        if (data.column.index === 7) { // GP
  ...
  // ── Summary Boxes ─────────────────────────────────────────────────────────
  const rsY    = doc.lastAutoTable.finalY + 6;
  const boxGap = 3;
  const boxW   = (W - 16 - boxGap * 2) / 3;
  const totalGP     = tableBody.reduce((a, r) => a + Number(r[7]), 0);
  const pctOverall  = totalFull > 0 ? ((totalObtained / totalFull) * 100).toFixed(1) : "0.0";

  // Row 1: Total Obtained Marks | Percentage of Marks (%) | Total GP
  const row1 = [
    { label: "Total Obtained Marks",   value: String(totalObtained),        fillColor: SKY_LT,    borderColor: SKY_MID, valueColor: SKY },
    { label: "Percentage of Marks (%)",value: `${pctOverall}%`,             fillColor: INDIGO_LT, borderColor: [199,210,254], valueColor: INDIGO },
    { label: "GPA",                     value: String(student.gpa),          fillColor: TEAL_LT,   borderColor: [153,234,214], valueColor: TEAL },
  ];
  const row1H = 18;
  row1.forEach((box, bi) => {
    const bx = 8 + bi * (boxW + boxGap);
    doc.setFillColor(...box.fillColor);
    doc.rect(bx, rsY, boxW, row1H, "F");
    doc.setDrawColor(...box.borderColor); doc.setLineWidth(0.5);
    doc.rect(bx, rsY, boxW, row1H, "S");
    doc.setFont("helvetica","normal"); doc.setFontSize(7.5); doc.setTextColor(...GREY);
    doc.text(box.label, bx + boxW/2, rsY + 5.5, { align: "center" });
    // Underline
    doc.setDrawColor(...box.borderColor); doc.setLineWidth(0.4);
    doc.line(bx + 8, rsY + 14, bx + boxW - 8, rsY + 14);
    doc.setFont("helvetica","bold"); doc.setFontSize(12); doc.setTextColor(...box.valueColor);
    doc.text(box.value, bx + boxW/2, rsY + 14, { align: "center" });
  });
  ...
  const row2Y    = rsY + row1H + 5;
  const row2H    = 20;

  const row2 = [
    { label: "Total GP",      value: String(totalGP.toFixed(1)), fillColor: SKY_LT, borderColor: SKY_MID, valueColor: SKY },
    { label: "LG",            value: gradeText,            fillColor: INDIGO_LT, borderColor: [199,210,254], valueColor: INDIGO },
    { label: "Merit Position",value: `#${myRank}`,         fillColor: AMBER_LT,  borderColor: [253,230,138], valueColor: AMBER },
  ];
  row2.forEach((box, bi) => {
    const bx = 8 + bi * (boxW + boxGap);
    doc.setFillColor(...box.fillColor);
    doc.rect(bx, row2Y, boxW, row2H, "F");
    doc.setDrawColor(...box.borderColor); doc.setLineWidth(0.5);
    doc.rect(bx, row2Y, boxW, row2H, "S");
    doc.setFont("helvetica","bold"); doc.setFontSize(13); doc.setTextColor(...box.valueColor);
    doc.text(box.label, bx + boxW/2, row2Y + 7, { align: "center" });
    // Underline
    doc.setDrawColor(...box.borderColor); doc.setLineWidth(0.4);
    doc.line(bx + 8, row2Y + 15, bx + boxW - 8, row2Y + 15);
    doc.setFont("helvetica","bold"); doc.setFontSize(15); doc.setTextColor(...box.valueColor);
    doc.text(box.value, bx + boxW/2, row2Y + 15, { align: "center" });
  });

  // ── Pass / Fail status strip ──────────────────────────────────────────────
  const isPass   = student.status === "PASS";
  const stripY   = row2Y + row2H + 6;
  doc.setFillColor(...(isPass ? GREEN : RED));
  doc.roundedRect(8, stripY, W-16, 10, 2, 2, "F");
  doc.setFont("helvetica","bold"); doc.setFontSize(9); doc.setTextColor(...WHITE);
  doc.text(isPass ? "PASS  —  Congratulations!" : "FAIL", W/2, stripY + 6.8, { align: "center" });

  // ── Signature Lines ───────────────────────────────────────────────────────
  const sigY = stripY + 18;
  doc.setDrawColor(...SKY_MID); doc.setLineWidth(0.4);
  doc.line(12, sigY, 70, sigY);
  doc.line(W-70, sigY, W-12, sigY);
  doc.setFont("helvetica","normal"); doc.setFontSize(7.5); doc.setTextColor(...GREY);
  doc.text("Student Signature",    41, sigY + 5, { align: "center" });
  doc.text("Authorized Signature", W-41, sigY + 5, { align: "center" });

  // ── Footer bar ────────────────────────────────────────────────────────────
  doc.setFillColor(...SKY_DK); doc.rect(5, H-13, (W-10)/2, 8, "F");
  doc.setFillColor(...SKY);    doc.rect(5+(W-10)/2, H-13, (W-10)/2, 8, "F");
  doc.setFont("helvetica","normal"); doc.setFontSize(6.5); doc.setTextColor(...WHITE);
  doc.text(`${school}  —  ${project.examName || "Progress Report"}`, 10, H-7.5);
  doc.text(`Generated: ${new Date().toLocaleDateString()}`, W-10, H-7.5, { align: "right" });
...
// ─── Firebase — bundled via npm, works in Brave/ad-blockers ──────────────────
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(firebase.auth, async (u) => {
      ...
      if (u) {
        const snap = await getDoc(doc(firebase.db, "users", u.uid));
        if (snap.exists() && snap.data().blocked) { setIsAccountBlocked(true); return; }
        try {
          const projQ = query(collection(firebase.db, "users", u.uid, "projects"), orderBy("createdAt", "asc"));
  ...
  const createProject = async () => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(firebase.db, "users", user.uid, "projects"), np);
      ...
      setSchoolName(""); setSchoolLocation(""); setClassLevel("");
  ...
  const deleteProject = async (id) => {
    ...
    try {
      await deleteDoc(doc(firebase.db, "users", user.uid, "projects", id));
  ...
  const renameProject = async (id, newName) => {
    ...
    try {
      await updateDoc(doc(firebase.db, "users", user.uid, "projects", id), { examName: newName.trim() });
  ...
  const addStudent = async () => {
    ...
    try {
      ...
      await updateDoc(doc(firebase.db, "users", user.uid, "projects", pr.id), {students: pr.students});
  ...
  return (
    <>
      ...
      {splash && <LogoSplash onDone={onSplashDone} />}
